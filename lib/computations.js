...
export function docsHighlight(contributors) {
  if (!contributors.length) return null;
  const sorted = [...contributors].sort((a, b) => b.docs_updated - a.docs_updated);
  const leader = sorted[0];
  const totalDocs = contributors.reduce((sum, item) => sum + item.docs_updated, 0);
  return {
    name: leader.name,
    handle: leader.handle,
    avatar: leader.avatar,
    docsUpdated: leader.docs_updated,
    share: totalDocs ? Number(((leader.docs_updated / totalDocs) * 100).toFixed(1)) : 0,
  };
}

export function docsUpdateCards(activityTimeline, contributors) {
  return activityTimeline
    .filter((item) => item.type === "docs_update")
    .slice(0, 8)
    .map((item, index) => {
      const contributor = contributors.find((person) => person.id === item.contributor);
      return {
        id: `doc-${item.date}-${index}`,
        title: `${item.count} page${item.count > 1 ? "s" : ""} updated`,
        date: item.date,
        contributor,
        summary: `${contributor?.name ?? "Contributor"} committed ${item.count} doc changes`,
        link: "#",
      };
    });
}
...
export function impactSummary(contributors, regions) {
  const totalUsers = contributors.reduce((sum, contributor) => sum + contributor.usage_impact, 0);
  const projectsHelped = contributors.length * 2;
  const avgImpact = contributors.length ? Math.round(totalUsers / contributors.length) : 0;
  return {
    totalUsers,
    projectsHelped,
    avgImpact,
...
export function projectImpactList(contributors) {
  return contributors.map((contributor) => ({
    id: contributor.id,
    project: `Project ${contributor.handle}`,
    summary: `${contributor.name} supported ${contributor.usage_impact} users across docs and reviews`,
  }));
}
